...
const LocalizationManager = {
    ...
    translations: {
        ...
        ru: {
            ...
            pressure_next_in: 'След. подъём через',
            continue_button: 'Продолжить',
            freeze_tool: 'Заморозка'
        },
        uz: {
            ...
            pressure_label: 'Bosim',
            pressure_next_in: "Keyingi ko'tarilish",
            continue_button: 'Davom etish',
            freeze_tool: 'Muzlatish'
        }
    },
...
// ============================================
// PRESSURE / RISING JUNK CONFIG (tuning only)
// ============================================
const PRESSURE_CONFIG = {
    initialRiseInterval: 12,          // Starting seconds between junk rises
    minRiseInterval: 5,               // Floor for rise interval after speedups
    speedupEverySeconds: 45,          // How often we accelerate pressure
    speedupFactor: 0.9,               // Multiply interval by this each speedup
    lineClearDelay1: 2,               // Extra seconds before next rise after any clear
    lineClearDelay3: 5,               // Extra seconds if 3+ lines cleared at once
    postContinueFreezeSeconds: 6,     // Invulnerability window after Continue
    freezePowerDuration: 5,           // Rewarded freeze power-up duration
    debugDisablePressure: false       // Set true for QA to disable the mechanic
};
    ...
    grantReward(rewardType) {
        switch (rewardType) {
            ...
            case 'freeze':
                ...
                break;
            case 'continue':
                window.dispatchEvent(new CustomEvent('activateContinue'));
                break;
        }
    },
...
// ============================================
// PRESSURE METER (UI helper)
// ============================================
const createPressureMeter = ({ valueEl, fillEl }) => {
    let baseInterval = PRESSURE_CONFIG.initialRiseInterval;

    const updateBar = (remainingSeconds) => {
        if (!fillEl || !baseInterval) return;
        const clamped = Math.max(0, Math.min(1, remainingSeconds / baseInterval));
        const filled = (1 - clamped) * 100;
        fillEl.style.height = `${Math.round(filled)}%`;
    };

    return {
        updateCountdown(remainingSeconds, currentInterval, paused = false) {
            if (typeof currentInterval === 'number' && currentInterval > 0) {
                baseInterval = currentInterval;
            }
            if (valueEl) {
                if (typeof remainingSeconds === 'number') {
                    valueEl.textContent = `${Math.max(0, Math.ceil(remainingSeconds))}s`;
                } else {
                    valueEl.textContent = '--';
                }
            }
            updateBar(typeof remainingSeconds === 'number' ? remainingSeconds : baseInterval);
            if (fillEl) {
                fillEl.classList.toggle('is-paused', paused);
            }
        },
        setFrozen(isFrozen) {
            if (fillEl) {
                fillEl.classList.toggle('is-frozen', isFrozen);
            }
        },
        reset() {
            if (valueEl) {
            ...
            if (fillEl) {
                ...
                fillEl.classList.remove('is-frozen', 'is-paused');
            }
        }
    };
};
...
// ============================================
// PRESSURE MANAGER - Rising junk rows
// ============================================
const createPressureManager = (scene, options) => {
    const cfg = PRESSURE_CONFIG;
    const meter = options.meter;
    let currentInterval = cfg.initialRiseInterval;
    let riseTimer = null;
    let speedupTimer = null;
    let countdownTimer = null;
    let freezeTimer = null;
    let nextRiseAt = null;
    let paused = cfg.debugDisablePressure;
    let active = !cfg.debugDisablePressure;

    const isGameOver = () => (typeof options.isGameOver === 'function') ? options.isGameOver() : false;

    const updateMeter = () => {
        if (!meter) return;
        const remainingSeconds = nextRiseAt ? Math.max(0, (nextRiseAt - scene.time.now) / 1000) : null;
        meter.updateCountdown(remainingSeconds, currentInterval, paused || !active);
    };

    const handleRise = () => {
        if (!active || paused || isGameOver()) {
            return;
        }
        const overflowed = options.onRise ? options.onRise() : false;
        if (overflowed) {
            stop();
    ...
    const scheduleNext = (delaySeconds) => {
        ...
        if (riseTimer) {
            riseTimer.remove(false);
        }
        const delayMs = Math.max(100, delaySeconds * 1000);
        nextRiseAt = scene.time.now + delayMs;
        riseTimer = scene.time.addEvent({
            delay: delayMs,
            callback: handleRise
        });
        updateMeter();
    ...
    const start = () => {
        if (cfg.debugDisablePressure) {
            return;
        }
        active = true;
        paused = false;
        scheduleNext(currentInterval);
        speedupTimer = scene.time.addEvent({
            delay: cfg.speedupEverySeconds * 1000,
            loop: true,
            callback: () => {
                const before = currentInterval;
                currentInterval = Math.max(cfg.minRiseInterval, currentInterval * cfg.speedupFactor);
                if (before !== currentInterval) {
                    updateMeter();
                }
            }
        });
    ...
    const stop = () => {
        ...
        paused = true;
        nextRiseAt = null;
        if (riseTimer) riseTimer.remove(false);
        if (speedupTimer) speedupTimer.remove(false);
        if (countdownTimer) countdownTimer.remove(false);
        if (freezeTimer) freezeTimer.remove(false);
        riseTimer = speedupTimer = countdownTimer = freezeTimer = null;
        if (meter) meter.reset();
    };

    const delayNextRise = (extraSeconds) => {
        if (!active || paused || !nextRiseAt) {
            return;
        }
        const remainingMs = Math.max(0, nextRiseAt - scene.time.now);
        scheduleNext(remainingMs / 1000 + extraSeconds);
    };

    const onLineClear = (linesCleared) => {
        if (!active || paused) {
            return;
        }
        if (linesCleared >= 3) {
            delayNextRise(cfg.lineClearDelay3);
        } else if (linesCleared >= 1) {
            delayNextRise(cfg.lineClearDelay1);
        }
    };
    ...
    const freeze = (seconds) => {
        ...
        paused = true;
        const remainingMs = nextRiseAt ? Math.max(0, nextRiseAt - scene.time.now) : currentInterval * 1000;
        if (riseTimer) {
            riseTimer.remove(false);
        }
        nextRiseAt = null;
        if (meter) {
            meter.setFrozen(true);
        }
        if (freezeTimer) {
            freezeTimer.remove(false);
        }
        freezeTimer = scene.time.delayedCall(seconds * 1000, () => {
            paused = false;
            if (meter) {
                meter.setFrozen(false);
            }
            scheduleNext(remainingMs / 1000);
        });
    };

    const getSecondsUntilRise = () => {
        if (!nextRiseAt) return null;
        return Math.max(0, Math.ceil((nextRiseAt - scene.time.now) / 1000));
    };
...
function create() {
    ...
    const domRestart = document.getElementById('restart-btn');
    const domContinue = document.getElementById('continue-btn');
    ...
    const domFreeze = document.getElementById('power-freeze');
    const domPressureValue = document.getElementById('pressure-timer');
    const domPressureFill = document.getElementById('pressure-fill');
    const domPressureLabel = document.getElementById('pressure-label');
    const pressureMeter = createPressureMeter({ valueEl: domPressureValue, fillEl: domPressureFill });
    let pressureManager = null;
    ...
    const showDomGameOver = () => {
        if (domModal) {
            ...
            if (domContinue) {
                domContinue.style.display = continueUsed ? 'none' : 'inline-flex';
                domContinue.disabled = continueUsed;
            }
        }
    };
    ...
    // --- Pressure helpers ---
    const buildJunkPattern = () => {
        const row = Array(gridSize).fill(false);
        let filled = 0;
        for (let x = 0; x < gridSize; x++) {
            if (Math.random() < 0.6) {
                row[x] = true;
        ...
        // Guarantee at least two empty cells
        let empties = gridSize - filled;
        while (empties < 2) {
            const idx = Phaser.Math.Between(0, gridSize - 1);
            if (row[idx]) {
                row[idx] = false;
                filled--;
                empties++;
        ...
        // Avoid a totally empty row
        if (filled === 0) {
            row[Phaser.Math.Between(0, gridSize - 1)] = true;
        }
        return row;
    };
    ...
    const applyJunkRise = () => {
        ...
        for (let y = 0; y < gridSize; y++) {
            for (let x = 0; x < gridSize; x++) {
                ...
                sprite.y -= cellSize;
                newGrid[targetY][x] = sprite;
            }
        }

        const pattern = buildJunkPattern();
        pattern.forEach((filled, x) => {
            if (!filled) return;
            const px = startX + x * cellSize + cellSize / 2;
            const py = startY + (gridSize - 1) * cellSize + cellSize / 2;
            ...
            newGrid[gridSize - 1][x] = sprite;
        });

        for (let y = 0; y < gridSize; y++) {
            for (let x = 0; x < gridSize; x++) {
                grid[y][x] = newGrid[y][x] || null;
            }
        }

        GameJuice.shakeScreen('light');
        updateShapeVisuals();
        return overflow;
    };
    ...
    const handlePressureOverflow = () => {
        ...
        stillActive.forEach((shape) => shape.disableInteractive && shape.disableInteractive());
    ...
    // Clear the highest occupied row (used for continue flow)
    const clearHighestOccupiedRow = () => {
        let targetRow = -1;
        for (let y = 0; y < gridSize; y++) {
        ...
        for (let x = 0; x < gridSize; x++) {
            const sprite = grid[targetRow][x];
            if (!sprite) continue;
            animateBlockRemoval(sprite);
            grid[targetRow][x] = null;
        }
        return true;
    };

    const resumeAfterContinue = () => {
        if (!isGameOver || continueUsed) {
            return;
        }
        continueUsed = true;
        setDeleteMode(false);
        ...
        clearHighestOccupiedRow();
        updateShapeVisuals();

        const stillActive = activeShapes.filter((shape) => shape && shape.active);
        stillActive.forEach((shape) => {
        ...
        if (pressureManager) {
            ...
            pressureManager.delayNextRise(PRESSURE_CONFIG.postContinueFreezeSeconds);
            pressureManager.freeze(PRESSURE_CONFIG.postContinueFreezeSeconds);
        }
    ...
    const checkAndClearLines = () => {
        ...
        if (pressureManager) {
            pressureManager.onLineClear(clearedLines);
        }
        return clearedLines;
    };
    ...
    const activateFreeze = () => {
        if (isGameOver || !pressureManager) {
            return;
        }
        pressureManager.freeze(PRESSURE_CONFIG.freezePowerDuration);
        GameJuice.shakeScreen('light');
    };
    ...
    window.addEventListener('activateFreeze', activateFreeze);
    window.addEventListener('activateContinue', activateContinue);
    ...
    if (domFreeze) {
        domFreeze.onclick = async () => {
            if (isGameOver) {
                return;
            }
            await YandexGamesSDK.showRewardAd('freeze');
        };
    }

    if (domContinue) {
        domContinue.onclick = async () => {
            if (isGameOver && !continueUsed) {
                await YandexGamesSDK.showRewardAd('continue');
            }
        };
    }
    ...
    pressureManager = createPressureManager(this, {
        isGameOver: () => isGameOver,
        onRise: applyJunkRise,
        onOverflow: handlePressureOverflow,
        meter: pressureMeter
    });
    ...
    if (!PRESSURE_CONFIG.debugDisablePressure && pressureManager) {
        pressureManager.start();
    } else {
        pressureMeter.reset();
    }
